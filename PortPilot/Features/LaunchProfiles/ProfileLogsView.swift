...
struct ProfileLogsView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Divider()
            if filtered.isEmpty {
                EmptyStateView(symbol: "text.alignleft", title: "No log output", message: "stdout, stderr, and lifecycle messages will stream here while PortPilot runs this profile.")
            } else {
                ScrollViewReader { proxy in
                    List(filtered) { entry in
                        HStack(alignment: .firstTextBaseline, spacing: PortPilotSpacing.small) {
