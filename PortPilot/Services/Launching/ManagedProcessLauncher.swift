...
actor ManagedProcessLauncher: ManagedProcessLaunching {
    ...
    func launch(_ profile: LaunchProfileConfiguration) async throws {
        guard profile.isReviewed else {
            throw PortPilotError.launchValidation("Review inferred command fields before launching this profile.")
        }
        guard running[profile.id]?.process.isRunning != true else {
            throw PortPilotError.launchValidation("\(profile.name) is already running under PortPilot.")
        }
        let issues = LaunchProfileValidator.validate(profile).filter { $0.severity == .error }
        guard issues.isEmpty else {
            throw PortPilotError.launchValidation(issues.map(\.message).joined(separator: " "))
        }
        ...
        for (name, reference) in profile.secretReferences {
            guard let value = try await secrets.value(for: reference) else { throw PortPilotError.missingSecret(name) }
        ...
        case .direct:
            guard let resolved = resolver.resolve(profile.command, environment: environment, workingDirectory: profile.workingDirectory) else {
                throw PortPilotError.commandUnavailable(profile.command)
            }
            executable = resolved
            arguments = profile.arguments
        case let .loginShell(path), let .custom(path):
            guard FileManager.default.isExecutableFile(atPath: path) else {
                throw PortPilotError.commandUnavailable(path)
            }
        ...
        do {
            ...
            throw PortPilotError.unexpected("\(profile.name) could not start: \(error.localizedDescription)")
        }
    ...
    func stop(profileID: UUID, timeoutSeconds: Double) async throws {
        ...
        guard !managed.process.isRunning else {
            throw PortPilotError.unexpected("The service did not stop before its graceful shutdown timeout. Force stop its verified process from Active Ports if needed.")
        }
