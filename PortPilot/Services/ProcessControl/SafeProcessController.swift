...
actor SafeProcessController: ProcessControlling {
    ...
    func terminate(_ process: ProcessMetadata, mode: TerminationMode) async throws -> TerminationOutcome {
        ...
        guard state == .validatingIdentity else { throw PortPilotError.unexpected("Termination could not begin.") }
        ...
        case let .force(confirmed):
            guard confirmed else {
                throw PortPilotError.unexpected("Force stop requires explicit confirmation.")
            }
            signal = SIGKILL
            timeout = 2
            modeName = "force"
        ...
        guard result.exitCode == 0 else {
            throw PortPilotError.commandFailed(command: "kill", status: result.exitCode, details: result.stderrString)
        }
