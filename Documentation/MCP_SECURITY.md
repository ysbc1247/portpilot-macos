...
Residual risk remains for any same-user client allowed to call approved MCP tools and for host state that changes after observation. Short leases, last-moment validation, exact ownership requirements, refusals, and bounded audit reduce but do not make the local machine atomic.
