...
final class HealthCheckTests: XCTestCase {
    func testHealthCheckAcceptsExpectedStatus() async throws {
        ...
        try await checker.waitUntilHealthy(
            configuration: .init(url: URL(string: "https://devberth.invalid/health")!, expectedStatus: 204, intervalSeconds: 0.01),
            timeoutSeconds: 0.5
        )
    }
    ...
    func testHealthCheckTimesOutOnWrongStatus() async {
        ...
        do {
            try await checker.waitUntilHealthy(
                configuration: .init(url: URL(string: "https://devberth.invalid/health")!, expectedStatus: 200, intervalSeconds: 0.01),
                timeoutSeconds: 0.05
            )
