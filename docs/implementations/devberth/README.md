...
Validated locally on 2026-07-22 with the repository's warnings-as-errors macOS test action: 181 of 181 tests passed with zero failures, skips, or expected failures in 107 seconds. The run covered nine harmless real-process integrations, 19 MCP control-plane tests, all UI tests, the observed-service confirmation and routed-owner deduplication regression, Escape dismissal of destructive confirmations, schema migrations, trust drift, Keychain lifecycle, process identity/group safety, deterministic ownership, and exact managed restart. Live QA against `/Applications/DevBerth.app` showed independent Stop controls beside Runtime, Project, and Managed Service rows; the Cloud Frontend observed-stop sheet dismissed with Escape without stopping the service. The installed MCP helper completed a protocol `2025-11-25` handshake, advertised 82 production tools, and returned a live runtime snapshot without error.
