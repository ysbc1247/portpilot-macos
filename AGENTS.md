...
- Keep one idempotent application-lifetime `PortMonitor` loop. Coalesce immediate refreshes behind any in-flight scan, suspend across system sleep, use visibility/stability-aware cadence, correlate Docker before semantic diffing, and never classify observation timestamps alone as runtime changes. Cache process metadata only behind a bounded native full-identity key that invalidates PID reuse, executable/argument identity, or current-directory changes.
