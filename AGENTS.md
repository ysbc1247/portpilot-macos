...
- Present an exact expected-port match as observed managed-service activity, never as DevBerth control authority. It may count as active in Projects, but Stop is available only from a live managed runtime; observed activity routes to Runtime inspection.
- Project bulk Start must preserve the full dependency graph while launching only stopped definitions; bulk Stop must preserve reverse dependency order while stopping only live DevBerth-controlled runtimes. Publish per-project progress and terminal results, prevent overlapping actions, and refresh runtime evidence after success.
