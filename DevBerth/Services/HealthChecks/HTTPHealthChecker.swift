...
struct HTTPHealthChecker: HealthChecking, Sendable {
    ...
    func waitUntilHealthy(configuration: HealthCheckConfiguration, timeoutSeconds: Double) async throws {
        ...
        if let lastError {
            DevBerthLogger.launching.error("Health check failed: \(lastError.localizedDescription, privacy: .public)")
        }
        throw DevBerthError.healthCheckTimedOut(configuration.url)
    }
}
