...
actor DockerCLIClient: DockerServing {
    ...
    func recentLogs(containerID: String, lines: Int) async throws -> String {
        ...
        guard result.exitCode == 0 else {
            throw DevBerthError.commandFailed(command: "docker logs", status: result.exitCode, details: Self.concise(result.stderrString))
        }
    ...
    private func runAction(_ arguments: [String], description: String) async throws {
        ...
        guard result.exitCode == 0 else {
            throw DevBerthError.commandFailed(command: "docker \(description)", status: result.exitCode, details: Self.concise(result.stderrString))
        }
    }

    private func dockerExecutable() throws -> URL {
        guard let executable else { throw DevBerthError.dockerUnavailable("The Docker CLI is not installed or is not on PATH.") }
        return executable
    }
