...
struct ProcessIdentityVerifier: ProcessIdentityVerifying, Sendable {
    ...
    func verify(_ expected: ProcessIdentity) async throws -> Bool {
        ...
        guard start.exitCode == 0, executable.exitCode == 0,
            ...
            throw DevBerthError.commandFailed(command: "process identity verification", status: max(start.exitCode, executable.exitCode), details: start.stderrString + executable.stderrString)
        }
        guard
